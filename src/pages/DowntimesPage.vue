      ...
      <Table>
        ...
        <TableBody>
          ...
          >
            ...
            <TableCell class="text-xs font-mono">{{ dt.startedAt.slice(0, 16).replace('T', ' ') }}</TableCell>
            <TableCell class="text-xs font-mono">{{ dt.endedAt?.slice(0, 16).replace('T', ' ') ?? 'открыт' }}</TableCell>
            <TableCell class="text-sm tabular-nums">{{ (dt.accountableDurationSeconds / 3600).toFixed(1) }} ч</TableCell>
            <TableCell>
            ...
            <TableCell class="text-sm font-medium tabular-nums">{{ dt.lossRubles.toLocaleString('ru-RU') }} ₽</TableCell>
          </TableRow>
        </TableBody>
      </Table>
    </CardContent>
