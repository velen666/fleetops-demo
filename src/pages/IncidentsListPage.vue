...
import { INCIDENT_STATUS_RU, INCIDENT_STATUS_CLASS, DOWNTIME_STATUS_RU } from '@/data/labels'
...
const { incidents, sites, robots, downtimes } = useDemoData()
...
const queues: Queue[] = [
  ...
  { key: 'no_coordinator', label: 'Без координатора', filter: (i) => i.status !== 'CLOSED' && !i.coordinatorName },
  { key: 'cause_unconfirmed', label: 'Причина не подтверждена', filter: (i) => i.status !== 'CLOSED' && i.causeMaturity !== 'FINAL' },
  { key: 'downtime_unconfirmed', label: 'Простой не подтверждён', filter: (i) => i.status !== 'CLOSED' && i.hasDowntime && !i.downtimeConfirmed },
  { key: 'waiting_service', label: 'Ожидают сервисных работ', filter: (i) => i.status === 'WAITING' },
  { key: 'need_recovery', label: 'Нужно подтвердить восстановление', filter: (i) => i.status !== 'CLOSED' && !i.recoveryConfirmed },
  { key: 'ready_to_close', label: 'Готовы к закрытию', filter: (i) => i.status === 'READY_TO_CLOSE' },
...
const filteredIncidents = computed(() => {
  ...
  if (searchText.value.trim()) {
    ...
    result = result.filter((i) => i.incidentNumber.toLowerCase().includes(s) || i.title.toLowerCase().includes(s) || i.description.toLowerCase().includes(s))
  }
...
const summary = computed(() => {
  ...
  return {
    count: list.length,
    confirmedDowntime: list.filter(i => i.downtimeConfirmed).reduce((s, i) => s + i.downtimeSeconds, 0),
...
function goTo(id: string): void { router.push({ name: 'incident-details', params: { incidentId: id } }) }
</script>
...
<template>
  <div class="space-y-4">
    ...
    <div class="flex flex-wrap gap-2">
      ...
      >
        {{ q.label }}
        <span class="ml-1 rounded-full px-1.5 text-xs tabular-nums" :class="activeQueue === q.key ? 'bg-primary-foreground/20' : 'bg-muted'">{{ queueCounts[q.key] }}</span>
      </Button>
    </div>
    ...
    <div class="flex flex-wrap items-end gap-3">
      ...
      <div class="space-y-1 flex-1 min-w-[200px]">
        ...
        <Input v-model="searchText" placeholder="Номер, название, описание..." />
      </div>
    </div>

    <!-- Summary -->
    <div class="flex flex-wrap gap-4 text-sm text-muted-foreground">
      <span>Инцидентов: <strong class="text-foreground tabular-nums">{{ summary.count }}</strong></span>
      <span>Подтверждённый простой: <strong class="text-foreground tabular-nums">{{ fmtDur(summary.confirmedDowntime) }}</strong></span>
      <span>Потери: <strong class="text-foreground tabular-nums">{{ summary.loss.toLocaleString('ru-RU') }} ₽</strong></span>
    </div>
    ...
    <Card>
      <CardContent class="p-0">
        <Table>
          ...
          <TableBody>
            <TableRow v-for="inc in filteredIncidents" :key="inc.id" class="row-interactive cursor-pointer" @click="goTo(inc.id)">
              <TableCell class="font-mono text-xs py-3 px-4">{{ inc.incidentNumber }}</TableCell>
              <TableCell class="text-xs py-3 px-4 max-w-xs">
                <p class="font-medium truncate">{{ incidentTypeLabel(inc.incidentTypeCode).split(' · ')[1] }}</p>
              ...
              <TableCell class="py-3 px-4">
                <span class="text-xs rounded px-1.5 py-0.5" :class="INCIDENT_STATUS_CLASS[inc.status]">{{ STATUS_RU[inc.status] }}</span>
              </TableCell>
              ...
              <TableCell class="text-xs py-3 px-4">
                <span v-if="inc.downtimeSeconds > 0" class="tabular-nums">{{ fmtDur(inc.downtimeSeconds) }}</span>
                <span v-else-if="inc.hasDowntime && !inc.downtimeConfirmed" class="text-warning">не подтв.</span>
                <span v-else class="text-muted-foreground">—</span>
              </TableCell>
              <TableCell class="text-xs tabular-nums py-3 px-4">
                <span v-if="inc.lossRubles > 0">{{ inc.lossRubles.toLocaleString('ru-RU') }} ₽</span>
                <span v-else class="text-muted-foreground">—</span>
              </TableCell>
