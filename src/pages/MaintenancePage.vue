...
const summary = computed(() => ({
  ...
  inProgress: maintenance.value.filter((m) => ['ASSIGNED', 'IN_PROGRESS', 'WAITING_PARTS'].includes(m.status)).length,
...
function robotName(id: string): string { return robots.value.find((r) => r.id === id)?.name ?? id }
function fmtDate(iso: string): string { return iso.slice(0, 10) }
function isOverdue(m: MaintenanceWork): boolean { return !m.completedAt && new Date(m.dueAt) < new Date() }

function openWork(m: MaintenanceWork): void { selected.value = m }
</script>
      ...
      <div class="space-y-1">
        ...
        <Select v-model="filterType">
          ...
          <SelectContent>
            ...
            <SelectItem v-for="(ru, code) in MAINTENANCE_TYPE_RU" :key="code" :value="code">{{ ru }}</SelectItem>
          </SelectContent>
        </Select>
      </div>
      <div class="space-y-1">
        ...
        <Select v-model="filterStatus">
          ...
          <SelectContent>
            <SelectItem value="all">Все</SelectItem>
            <SelectItem v-for="(ru, code) in MAINTENANCE_STATUS_RU" :key="code" :value="code">{{ ru }}</SelectItem>
          </SelectContent>
        </Select>
      </div>
    </div>
    ...
    <Card>
      <CardContent class="p-0">
        <Table>
          ...
          <TableBody>
            <TableRow v-for="m in filtered" :key="m.id" class="row-interactive cursor-pointer" @click="openWork(m)">
              <TableCell class="py-3 px-4">
                <span class="text-xs rounded px-1.5 py-0.5" :class="m.type === 'EMERGENCY' ? 'bg-destructive/15 text-destructive' : m.type === 'PLANNED' ? 'bg-primary/15 text-primary' : 'bg-muted text-muted-foreground'">{{ MAINTENANCE_TYPE_RU[m.type] }}</span>
              </TableCell>
              ...
              <TableCell class="text-xs py-3 px-4" :class="isOverdue(m) ? 'text-destructive font-medium' : ''">
                {{ fmtDate(m.dueAt) }}
                <span v-if="isOverdue(m)" class="block text-destructive">просрочено</span>
              </TableCell>
              <TableCell class="py-3 px-4">
                <span class="text-xs rounded px-1.5 py-0.5" :class="STATUS_CLASS[m.status]">{{ MAINTENANCE_STATUS_RU[m.status] }}</span>
              </TableCell>
    ...
    <Dialog :open="!!selected" @update:open="(v) => !v && (selected = null)">
      <DialogContent class="max-w-xl">
        <DialogHeader class="pb-4 border-b border-border">
          ...
          <DialogDescription>{{ MAINTENANCE_TYPE_RU[selected?.type ?? ''] }} · {{ MAINTENANCE_STATUS_RU[selected?.status ?? ''] }}</DialogDescription>
        </DialogHeader>
        <div v-if="selected" class="space-y-4 text-sm">
          <div class="grid grid-cols-2 gap-3">
            <div><p class="text-xs text-muted-foreground">Робот</p><p>{{ robotName(selected.robotId) }}</p></div>
            <div><p class="text-xs text-muted-foreground">Объект</p><p>{{ siteName(selected.siteId) }}</p></div>
            <div><p class="text-xs text-muted-foreground">Исполнитель</p><p>{{ selected.executor }}</p></div>
            <div><p class="text-xs text-muted-foreground">Срок</p><p :class="isOverdue(selected) ? 'text-destructive' : ''">{{ fmtDate(selected.dueAt) }}</p></div>
            <div v-if="selected.completedAt"><p class="text-xs text-muted-foreground">Выполнено</p><p>{{ fmtDate(selected.completedAt) }}</p></div>
          </div>
          ...
          <div v-if="selected.incidentId" class="border-t pt-3">
            <button class="text-primary hover:underline text-sm" @click="router.push({ name: 'incident-details', params: { incidentId: selected.incidentId } }); selected = null">
