...
const { stats, incidents, downtimes, analytics, robots, sites, costRates } = useDemoData()
...
const siteName = computed(() => selectedSite.value === 'all' ? 'Все объекты' : sites.value.find(s => s.id === selectedSite.value)?.name ?? '')

const filteredIncidents = computed(() => selectedSite.value === 'all' ? incidents.value : incidents.value.filter(i => i.siteId === selectedSite.value))
const filteredDowntimes = computed(() => selectedSite.value === 'all' ? downtimes.value : downtimes.value.filter(d => d.siteId === selectedSite.value))
...
const filteredStats = computed(() => {
  ...
  const unclassified = incs.filter(i => i.causeMaturity === 'NONE' || i.causeCode === 'CA-060').length
  return { totalDowntime, totalLoss, active, unclassified, total: incs.length, classified: incs.length - unclassified }
})
...
const topCauses = computed(() => {
  ...
  for (const inc of filteredIncidents.value) {
    if (inc.lossRubles > 0 && inc.causeCode) {
      ...
      e.count++; e.loss += inc.lossRubles; e.hours += inc.downtimeSeconds / 3600
...
const robotStats = computed(() => {
  const map = new Map<string, { name: string; model: string; site: string; count: number; downtime: number; loss: number }>()
  for (const r of robots.value) {
    if (selectedSite.value !== 'all' && r.siteId !== selectedSite.value) continue
    map.set(r.id, { name: r.name, model: r.model, site: sites.value.find(s => s.id === r.siteId)?.name ?? r.siteId, count: 0, downtime: 0, loss: 0 })
  }
  for (const inc of filteredIncidents.value) {
    if (!inc.robotId) continue
    const e = map.get(inc.robotId)
    if (e) { e.count++; e.downtime += inc.downtimeSeconds; e.loss += inc.lossRubles }
  }
...
function exportReport(): void {
  const lines = [
    ...
    '',
    '=== ТОП ПРИЧИН ===',
    ...topCauses.value.map((c, i) => `  ${i+1}. ${causeLabel(c.code)}: ${c.count} случаев, ${(c.hours).toFixed(1)} ч, ${c.loss.toLocaleString('ru-RU')} ₽`),
    '',
    '=== SLA ===',
    ...
    '',
    '=== ПО РОБОТАМ ===',
    ...robotStats.value.map(r => `  ${r.name} (${r.model}, ${r.site}): ${r.count} инц., ${(r.downtime/3600).toFixed(1)} ч, ${r.loss.toLocaleString('ru-RU')} ₽`),
    '',
    '=== СТАВКИ ===',
  ...
  a.href = url; a.download = `fleetops-report-${new Date().toISOString().slice(0, 10)}.txt`
  a.click(); URL.revokeObjectURL(url)
  toast.success('Отчёт экспортирован')
}

function openBreakdown(title: string): void {
  breakdownTitle.value = title; showBreakdown.value = true
}
</script>
...
<template>
  <div class="space-y-4">
    ...
    <div class="flex items-center gap-3">
      ...
      <Button v-if="auth.can('reports.export')" variant="outline" size="sm" class="ml-auto mt-5" @click="exportReport">
    ...
    <!-- Summary KPIs -->
    <Card>
      <CardHeader><CardTitle>Управленческий отчёт — {{ siteName }}</CardTitle></CardHeader>
      <CardContent>
        <div class="grid grid-cols-2 md:grid-cols-5 gap-3">
          <div class="kpi-clickable border border-border rounded-lg p-3" @click="openBreakdown('Доступность')">
            <p class="text-muted-foreground text-xs">Доступность</p>
            <p class="text-xl font-bold tabular-nums">{{ (100 - (filteredStats.totalDowntime / (30*24*3600)) * 100).toFixed(1) }}%</p>
          </div>
          <div class="kpi-clickable border border-border rounded-lg p-3" @click="openBreakdown('Простой')">
            <p class="text-muted-foreground text-xs">Простой</p>
            <p class="text-xl font-bold tabular-nums">{{ (filteredStats.totalDowntime / 3600).toFixed(1) }} ч</p>
          </div>
          <div class="kpi-clickable border border-border rounded-lg p-3" @click="openBreakdown('Потери')">
            <p class="text-muted-foreground text-xs">Потери</p>
            <p class="text-xl font-bold tabular-nums">{{ filteredStats.totalLoss.toLocaleString('ru-RU') }} ₽</p>
          </div>
          ...
          <div class="border border-border rounded-lg p-3">
            ...
            <p class="text-xl font-bold tabular-nums text-warning">{{ filteredStats.unclassified }}</p>
          </div>
        </div>
      </CardContent>
    </Card>
    ...
    <Card>
      ...
      <CardContent class="p-0">
        <Table>
          ...
          <TableBody>
            <TableRow v-for="c in topCauses" :key="c.code" class="row-interactive cursor-pointer"
              @click="router.push({ name: 'analytics' })">
              <TableCell class="text-sm py-3 px-4">{{ causeLabel(c.code) }}</TableCell>
              <TableCell class="text-xs py-3 px-4">{{ CAUSE_CATALOG[c.code]?.zone ?? '—' }}</TableCell>
              ...
              <TableCell class="text-sm font-medium tabular-nums py-3 px-4">{{ c.loss.toLocaleString('ru-RU') }} ₽</TableCell>
            </TableRow>
          </TableBody>
        </Table>
      </CardContent>
        ...
        <Table>
          ...
          <TableBody>
            <TableRow v-for="rs in robotStats" :key="rs.name" class="row-interactive">
              ...
              <TableCell class="text-sm tabular-nums py-3 px-4">{{ (rs.downtime / 3600).toFixed(1) }} ч</TableCell>
              <TableCell class="text-sm font-medium tabular-nums py-3 px-4">{{ rs.loss.toLocaleString('ru-RU') }} ₽</TableCell>
            </TableRow>
          </TableBody>
        </Table>
      </CardContent>
      ...
      <Card>
        ...
        <CardContent>
          <div class="flex gap-6">
            <div class="text-center"><p class="text-3xl font-bold text-success tabular-nums">{{ analytics.sla.reactionMet }}</p><p class="text-xs text-muted-foreground">в норме</p></div>
            <div class="text-center"><p class="text-3xl font-bold text-destructive tabular-nums">{{ analytics.sla.reactionViolated }}</p><p class="text-xs text-muted-foreground">нарушено</p></div>
          </div>
        </CardContent>
      </Card>
      <Card>
        ...
        <CardContent>
          <div class="flex gap-6">
            <div class="text-center"><p class="text-3xl font-bold text-success tabular-nums">{{ analytics.sla.recoveryMet }}</p><p class="text-xs text-muted-foreground">в норме</p></div>
            <div class="text-center"><p class="text-3xl font-bold text-destructive tabular-nums">{{ analytics.sla.recoveryViolated }}</p><p class="text-xs text-muted-foreground">нарушено</p></div>
          </div>
        </CardContent>
      </Card>
    </div>
      ...
      <DialogContent class="max-w-2xl">
        ...
        <Table>
          <TableHeader><TableRow>
            ...
            <TableHead>Простой</TableHead><TableHead>Потери</TableHead>
          </TableRow></TableHeader>
          <TableBody>
            <TableRow v-for="inc in filteredIncidents.filter(i => i.lossRubles > 0)" :key="inc.id" class="row-interactive cursor-pointer"
              ...
              <TableCell class="text-xs py-2 px-4">{{ incidentTypeLabel(inc.incidentTypeCode) }}</TableCell>
              <TableCell class="text-xs py-2 px-4">{{ causeLabel(inc.causeCode) }}</TableCell>
              <TableCell class="text-xs tabular-nums py-2 px-4">{{ (inc.downtimeSeconds / 3600).toFixed(1) }} ч</TableCell>
